			...
			case 'result':
				{
					...
					if (e.data.finished) playLoading(false);
				}
				break;
		}
	};
...
let openLetter = (id, saveToHistory) => {
	...
	if (saveToHistory) history.pushState({}, 'Day ' + id, '#day' + id);

	if (curState === STATE.BROWSE) curState = STATE.SVG;
	else {
...
let showLetter = () => {
	...
	let sourceURL =
		'https://github.com/BretHudson/advent-of-code/blob/main/2016/js/day/' +
		curDay +
		'.js';
...
let placeHeaders = () => {
	window.requestAnimationFrame(() => {
		...
		let rect = document
			.getElementById('day-holder-wrapper')
...
let resizeDayHolder = () => {
	...
	let extraPadding = 10;
	let wrapperHeight =
		dayHolder.getBoundingClientRect().height + extraPadding * 2 + 'px';
	...
	window.requestAnimationFrame(() => {
		...
		dayHolder.style.paddingLeft = dayHolder.style.paddingRight =
			sidePadding + 'px';
	});
};
		...
		case STATE.LETTER:
			{
				...
				if (e.path[0] !== overlay) {
					...
					touchStartLetterTransform =
						transformPositions[letterTransformPos];
...
window.addEventListener('touchmove', (e) => {
	if (curState === STATE.LETTER) e.preventDefault();
