...
const daysCompleted = 4;

const minYear = 2015;
const maxYear = (new Date(Date.now() + 30 * 8.64e7)).getFullYear() - 1;

const computeNaughtyScore = name => {
	return 12 - (name.toUpperCase().replace(/\s/g, '').split('').reduce((acc, l) => acc = (acc + l.charCodeAt(0)) % 64, 0) * 487 % 13);
};
...
const startWork = id => {
	...
	let href = 'https://github.com/BretHudson/AdventOfCode2019';
	if (id !== null) {
		text = `View Day ${id} Source`;
		href += `/blob/master/js/day/${id}.js`;
	}
...
window.on('DOMContentLoaded', e => {
	...
	addToFooter('https://github.com/BretHudson/AdventOfCode2019', 'View Source');
	for (let year = 2015; year <= maxYear; ++year) {
		if (year === 2019) continue;
		addToFooter(`https://brethudson.github.io/AdventOfCode${year}/`, year);
	}
