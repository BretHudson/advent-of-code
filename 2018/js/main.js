			...
			case 'ascii':
				{
					addLog(
						'output',
						'Credits to [https://asciiart.website/index.php?art=holiday/christmas/other](asciiart.website) for the banner ASCII art.',
					);
				}
				break;
			...
			case 'goto':
				{
					...
					if (targetYear === 2018)
						addLog(
							'output',
							`${STR_ERROR_GRAY} You're already at Bret Hudson's Advent of Code 2018!`,
						);
					else if (targetYear >= minYear && targetYear <= maxYear) {
						addLog(
							'output',
							`Redirecting to Bret Hudson's Advent of Code ${targetYear}`,
						);
						window.setTimeout(() => {
							window.location.assign(
								`https://advent-of-code.brethudson.com/${targetYear}/`,
							);
						}, 2000);
					} else
						addLog(
							'output',
							`${STR_ERROR_RED} ${targetYear} is out of bounds!`,
						);
				}
				break;
			...
			case 'long':
				{
					...
					if (parts.length > 1) {
						if (parts[1] >= 0 && parts[1] < str.length)
							...
							addLog(
								'output',
								`${STR_ERROR_RED} x must be between 0 and ${
									str.length - 1
								}`,
							);
					} else {
			...
			case 'how':
				{
					addLog(
						'output',
						'This site was created using HTML, CSS, and JavaScript/ES6. Source code is available on [https://github.com/BretHudson/advent-of-code/tree/main/2018](GitHub).',
					);
					resolve(true);
				}
				break;
			...
			case 'view':
				{
					addLog(
						'output',
						'The source for this site can be found on [https://github.com/BretHudson/advent-of-code/tree/main/2018](GitHub).',
					);
					resolve(true);
				}
				break;
			...
			case 'what':
				{
					addLog(
						'output',
						"This site is a command line/terminal with [https://brethudson.com](Bret Hudson)'s solutions for [https://adventofcode.com/2018](Advent of Code 2018).",
					);
					resolve(true);
				}
				break;
			...
			case 'who':
				{
					addLog(
						'output',
						'This site was created by [https://brethudson.com](Bret Hudson).',
					);
					resolve(true);
				}
				break;
			...
			case 'why':
				{
					addLog(
						'output',
						'This site was created for [https://adventofcode.com/2018](Advent of Code 2018).',
					);
					resolve(true);
				}
				break;
						...
						case 'copy':
							{
								...
								if (id === 1 || id === 2) {
									...
									addLog(
										'output',
										`Copied "[c:gray]${str}[/c]" to clipboard.`,
									);
								} else {
						...
						case 'input':
							{
								addLog(
									'output-more',
									`Please insert your [c:white]input[/c]:`,
								);
						...
						case 'view':
							{
								addLog(
									'output',
									`[https://github.com/BretHudson/advent-of-code/blob/main/2018/js/day/${workerDay}.js](Day ${workerDay} Source).`,
								);
								resolve(true);
							}
							break;
...
let updateLog = (id, str, append = false) => {
	if (append) document.q(`.log[id=log-${id}] span`).textContent += str;
	else document.q(`.log[id=log-${id}] span`).textContent = str;
};
...
class LoadingBar {
	...
	draw() {
		...
		if (this.percent < 100) str += this.icons[this.curIcon];
		str +=
			'.'.repeat(Math.max(0, 19 - solid)) +
			`] ${this.mask || this.percent}%`;
	...
	drawElapsed() {
		...
		addLog(
			'output-more',
			`Execution took approximately ${(elapsed / 1000).toFixed(3)}s`,
		);
		addLog('empty', '');
	}
...
let sendCommand = async (str) => {
	...
	if (recognized === false) didNotRecognizeLog();
...
let didNotRecognizeLog = () => {
	...
	addLog(
		'output',
		`${STR_ERROR_GRAY} Did not recognize [c:gray]${command}[/c] as a command. Type [c:gray]help[/c] for assistance.`,
	);
};
...
let addLog = (type, ...contents) => {
	...
	let log = $new('.log.' + type)
		.id('log-' + id)
		.attr('data-time', dateStr);
	if (type !== 'input') {
		log.init((div) => {
			div.innerHTML = div.innerHTML.replace(
				/\[([\w\s\:\/\.\?\-=]+)\]\(([\w\s\.]+)?\)/g,
				(match, url, text) => `<a href="${url}">${text}</a>`,
			);
		});
	}
	...
	while (contents.length > 0) {
		...
		if (type !== 'input') {
			if (colorMatch !== null) {
				...
				if (parts[0] !== '') {
					cur = parts[0];
					if (parts[1] !== '') contents.unshift(parts[1]);
...
let printWelcome = () => {
	...
	addLog(
		'output',
		`[c:white]Welcome to my[/c] [c:red]Advent of Code 2018[/c] [c:white]solutions![/c]`,
	);
	addLog('empty', '');

	addLog(
		'output-more',
		'My name is [c:orange]Bret Hudson[/c], and this site houses my solutions to the [https://adventofcode.com/2018](Advent of Code 2018) challenge written in [c:yellow]JavaScript/ES6[/c].',
	);
	addLog('empty', '');

	addLog(
		'output-more',
		'This site works best on [c:blue]G[/c][c:red]o[/c][c:yellow]o[/c][c:blue]g[/c][c:green]l[/c][c:red]e[/c] [c:yellow]Chrome[/c].',
	);
	addLog('empty', '');

	addLog(
		'output-more',
		'If you reload the page, or navigate away, your logs will be saved. :)',
	);
	addLog('empty', '');

	addLog(
		'output-more',
		`Lost? Type [c:gray]help[/c] and hit enter in the textbox below.`,
	);
};
...
let readLogs = () => {
	...
	if (logs.innerHTML === '') {
		printWelcome();
		addLog('empty', '');
	} else logID = 1 + +logs.lastChild.id.replace('log-', '');
